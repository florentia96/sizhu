...
export const ascEngine: FeatureEngine = {
  build(vals: string[]): Section[] {
    ...
    secs.push({
      ...
      meta: `คำนวณจากตำแหน่งดาวจริง (tropical zodiac) · ลัคนาเปลี่ยนราศีทุก ~2 ชั่วโมง เวลาเกิดที่แม่นยำจึงสำคัญมาก`,
    });
    ...
    secs.push({
      ...
      accent: STAR,
      cells: [
        { name: "ลัคนา (Asc · tropical)", value: `ราศี${ascRasi} ${degStr(asc.deg)}`, note: `เจ้าเรือนลัคนา: ${RASI_RULER[ascRasi]} · ธาตุ${ascEl}` },
        ...
        { name: "จุดกลางฟ้า (MC · Midheaven)", value: `ราศี${mcRasi} ${degStr(mc.deg)}`, note: "ทิศทางอาชีพ ชื่อเสียง เป้าหมายต่อสาธารณะ" },
        { name: "ลัคนาโหราไทย (sidereal)", value: `ราศี${lagna.rasi} ${degStr(lagna.deg)}`, note: "ปรับด้วย Lahiri ayanamsa (ระบบโหราไทย)" },
      ],
    });

    secs.push({
      kind: "prose",
      title: "เรือนสำคัญ — อ่านจากลัคนาแบบจักรราศี (Whole-sign)",
      glyph: "宮",
      accent: STAR,
      paras: [
        {
          h: `เรือน 1 (ตัวตน) = ราศี${ascRasi}`,
          t: `ระบบ Whole-sign ใช้ราศีลัคนาเป็นเรือน 1 เต็มราศี แล้วนับราศีถัดไปเป็นเรือน 2, 3 ... ไปจนครบ 12 เรือน เป็นวิธีอ่านเรือนที่โหราศาสตร์ไทยและอินเดียใช้เป็นหลัก`,
        },
        {
    ...
    secs.push({
      ...
      accent: GOLD,
      paras: [
        { t: "ลัคนาตะวันตกใช้จักรราศีแบบ tropical (อิงฤดูกาลและจุดวิษุวัต) ส่วนโหราไทยใช้จักรราศีแบบ sidereal (อิงกลุ่มดาวจริงบนท้องฟ้า) ปรับด้วยค่า ayanamsa แบบ Lahiri ปัจจุบันสองระบบต่างกันราว 24 องศา ลัคนาจึงมักตกคนละราศีกัน เลือกอ่านตามสำนักที่ยึดถือ ทั้งสองไม่ได้ขัดแย้งกัน เพียงใช้คนละมาตรวัด" },
      ],
    });

    secs.push({
      kind: "note",
      text: "ลัคนาและราศีจันทร์คำนวณจากตำแหน่งดาวจริง ร่วมกับเวลาและพิกัดเมืองเกิด เวลาเกิดคลาดเคลื่อนเพียงไม่กี่นาทีอาจทำให้ลัคนาเปลี่ยนราศีได้ จึงควรใช้เวลาที่แม่นยำที่สุด ระบบใช้ standard timezone ไม่รวม DST",
    });
